...
(function () {
  ...
  function loadPortfolioCarousel() {
    ...
    supportedPortfolioFiles.forEach((file, index) => {
      ...
      if (isVideo) {
        card.innerHTML = `
          <div class="relative w-full h-full bg-[rgba(255,240,250,0.85)] flex items-center justify-center">
            <video 
              src="${file}" 
            ...
            ></video>
            <div class="absolute inset-0 bg-gradient-to-t from-[#2d1b4e]/40 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            <button class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10" onclick="toggleVideoPlay(this)">
              <svg class="w-12 h-12 text-[#ff2d78]" fill="currentColor" viewBox="0 0 24 24">
                <path d="M8 5v14l11-7z"/>
              </svg>
            </button>
            <button class="absolute bottom-4 right-4 w-10 h-10 flex items-center justify-center rounded-full glass-card text-[#2d1b4e] hover:text-[#ff2d78] transition-all duration-300 z-20" onclick="toggleVideoMute(this)" title="Увімкнути звук">
        ...
        card.innerHTML = `
          <div class="relative w-full h-full bg-[rgba(255,240,250,0.85)]">
            <img 
              src="${file}" 
            ...
            >
            <div class="absolute inset-0 bg-gradient-to-t from-[#2d1b4e]/30 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
          </div>
        `;
      } else {
